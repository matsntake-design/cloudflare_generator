...
OUT_DIR = ROOT / 'output'
POPULAR_DIR = OUT_DIR / 'v1' / 'popular'
...
def clear_popular_pages() -> None:
    if not POPULAR_DIR.exists():
        return
    for page_file in POPULAR_DIR.glob('page-*.json'):
        page_file.unlink()
...
def main() -> None:
    ...
    clear_popular_pages()
    ensure_dir(POPULAR_DIR)

    page_count = max(1, math.ceil(len(popular_articles) / PAGE_SIZE)) if popular_articles else 1
    for page in range(1, page_count + 1):
        payload = to_page_payload(popular_articles, page, generated_at)
        write_json(POPULAR_DIR / f'page-{page}.json', payload)

    meta = {
        'version': 1,
        'generatedAt': generated_at,
        'popularPageCount': page_count,
    }
    if META_FILE.exists():
        try:
    ...
    print('Generated popular pages under:', POPULAR_DIR)

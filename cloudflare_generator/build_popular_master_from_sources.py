...
# popular 専用の軽量更新ルート。
# latest より少し広めに取り、24時間ウィンドウに必要な候補を保ちやすくする。
POPULAR_MAX_ARTICLES_PER_SITE = 16
POPULAR_MAX_CANDIDATES_PER_SITE = 120
...
def merge_articles(refreshed_articles: list[dict], existing_articles: list[dict]) -> list[dict]:
    # popular は 24h の候補母集団が欲しいので、fresh を優先しつつ既存も残す。
...
def fetch_popular_articles_for_targets(target_sites: list[dict]) -> list[dict]:
    collected: list[dict] = []
    with patched_fetch_limits(
        max_articles_per_site=POPULAR_MAX_ARTICLES_PER_SITE,
        max_candidates_per_site=POPULAR_MAX_CANDIDATES_PER_SITE,
    ):
...
def main() -> int:
    ...
    refreshed_articles = fetch_popular_articles_for_targets(target_sites)
    if not refreshed_articles:
        print('[WARN] popular 軽量更新では新しい記事を取得できませんでした。既存 master を維持します。')
        return 1
    ...
    print(
        'Note: popular 専用の軽量更新です。標準サイトの先頭側を少し広めに取得し、'
        '既存 master とマージして popular pages の鮮度を上げています。'
    )
    return 0
